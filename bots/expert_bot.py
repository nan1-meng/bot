...
class ExpertBot(BaseBot):
    def __init__(self, platform, api_key, secret_key, symbol, config, user_id, key_id, key_service,
        ...
        super().__init__(platform, api_key, secret_key, symbol, config, user_id, key_id,
                         mode_display, callback, auto_warmup=True)

        self.indicators = Indicators()
        self.signal_engine = SignalEngine()
        self.risk_manager = RiskManager(key_id, user_id)  # 学习器稍后注入
    ...
    def _has_tradable_position(self, price: float) -> bool:
        if not self.position["has_position"]:
            return False
        return self._position_value(price) >= self.min_order_value

    def _has_dust_position(self, price: float) -> bool:
        if not self.position["has_position"]:
            return False
        return 0 < self._position_value(price) < self.min_order_value
    ...
    def _should_buy(self, usdt_balance: float, price: float) -> bool:
        ...
        # 只有“可交易持仓”才阻止新买入
        # 小于最小交易额的残仓不阻止买入
        if self.position["has_position"] and self._has_tradable_position(price):
            self._set_action("hold", "已有可交易持仓，禁止新开买入")
            return False
    ...
    def _execute_buy(self, usdt_balance: float, price: float, now: float) -> bool:
        ...
        try:
            ...
            with self._position_lock:
                ...
                # 如果之前是残仓，现在买入后恢复正式仓位
                if old_qty == 0 or self._position_value(price) >= self.min_order_value:
                    self.add_count = 0
    ...
    def _execute_sell(self, sell_orders: List[Tuple[float, float]], now: float) -> bool:
        ...
        # 如果部分计划卖出小于最小额，但全仓价值>=最小额，则改为整仓卖出，避免被卡死
        ...
        try:
            ...
            if hasattr(self.risk_manager, 'online_learner') and self.risk_manager.online_learner:
                try:
                    ...
                    self._log("系统", f"{self.symbol} 学习器回调失败: {e}")

            with self._position_lock:
                self.position["qty"] -= raw_qty

                # 只有真正清空时才清零；残仓保留，不强制抹掉
    ...
    # =========================
    def _run_loop(self):
        self._log("系统", f"{self.symbol} AI模式启动（自适应动态策略）")
        self.running = True

        initial_balance, _ = self.get_balances()
        self.session_id = start_bot_session(
            user_id=self.user_id,
            key_id=self.key_id,
            symbol=self.symbol,
            bot_mode="expert",
        ...
        while self.running and not self._stop_event.is_set():
            ...
            if now - self.last_heartbeat >= 30:
                ...
                self._log("系统", f"{self.symbol} AI模式心跳")
            ...
            try:
                ...
                # 定期打印评分
                if now - self.last_score_log_time >= 60:
                ...
                # 3. 残仓：不进入正常卖出/补仓，只允许进入买入逻辑
                if dust_position:
                    self._set_action("hold", f"残仓观察 {position_value:.2f}U，小于最小交易额，不卖不补，可继续等待买入条件")
                ...
                # 5. 状态日志
                if now - self.last_status_log >= 120:
                ...
                self._log("系统", f"{self.symbol} 交易线程异常: {e}")
                time.sleep(5)

        final_balance, _ = self.get_balances()
        end_bot_session(self.session_id, final_balance)