...
class GlobalSellBot(BaseBot):
    ...
    def _process_tick(self, price: float, now: float):
        if not self.position.get("has_position", False):
            return
        ...
        if hold_time > self.max_hold_hours * 3600 and net_profit_pct > 0:
            self._log("系统", f"{self.symbol} 持仓超过{self.max_hold_hours}小时且盈利，保底卖出")
            self._sell_all(raw_qty, price)
            return

        # 盈利保护
        if self.highest_price_since_entry > 0 and net_profit_pct > 0:
        ...
        # 移动止盈
        if net_profit_pct > 0 and self.highest_price_since_entry > 0:
            drawdown = (self.highest_price_since_entry - price) / self.highest_price_since_entry
    ...
    def _sell_all(self, qty, price):
        try:
            ...
            self._after_sell(pnl=pnl, exit_reason="stop_loss" if pnl < 0 else "take_profit",
                             hold_seconds=int(time.time() - self.entry_time), market_state="unknown")
            if self.callback:
    ...
    def _sell_partial(self, qty, price):
        try:
            ...
            self._after_sell(pnl=pnl, exit_reason="take_profit_partial",
                             hold_seconds=int(time.time() - self.entry_time), market_state="unknown")
            if self.callback:
                self.callback(self.key_id, self.symbol, "卖出", price, time.time())
