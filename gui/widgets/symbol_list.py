...
class SymbolList(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        columns = ('币种', '状态', '模式', '持仓', '成本价', '最新价', '浮动盈亏', '总盈亏')
        ...
        self.tree.column('成本价', width=130)
        self.tree.column('最新价', width=130)
        self.tree.column('浮动盈亏', width=150)
        self.tree.column('总盈亏', width=150)
    ...
    def set_key(self, key_id: int):
        ...
        self.key_service.set_selected_key(key_id)

        def sync_all():
            key = self.key_service.get_key(key_id)
            if key:
                for symbol in key['symbols'].keys():
                    self.key_service.sync_trade_history_for_symbol(key_id, symbol, since_days=30)

        threading.Thread(target=sync_all, daemon=True).start()
        self.key_service.update_assets(key_id, force=True)
        if self.winfo_exists():
            self.refresh()
        self._stop_refresh_timer()
        self._refresh_timer = self.after(5000, self._periodic_refresh)

    def _periodic_refresh(self):
        if self.selected_key_id and self.winfo_exists():
            self.refresh()
    ...
    def refresh(self):
        ...
        expected_symbols = set(assets.keys())
        current_items = {}
        ...
        for symbol in sorted(expected_symbols):
            ...
            price = asset['price']
            cost = asset['cost']
            pnl_unrealized = asset['pnl_usdt']
            pnl_pct = asset['pnl_pct']
            total_pnl = asset.get('total_pnl', 0.0)
            ...
            if symbol in configured:
                ...
                if bot_status and bot_status['running']:
                    ...
                    status = '停止'
                    cfg_mode = key['symbols'].get(symbol, {}).get('mode', '')
                    if cfg_mode == 'global':
                        mode = '全局模式'
                    else:
                        mode = '-'
            else:
                status = '持仓' if qty > 0 else '未持仓';
                mode = '-'

            qty_str = f'{qty:.4f}' if qty > 0 else '0'
            price_str = f'{price:.6f}' if price > 0 else '-'
            cost_str = f'{cost:.6f}' if cost > 0 else '-'
            pnl_unrealized_str = f'{pnl_unrealized:.2f} ({pnl_pct:.2f}%)' if (
                        pnl_unrealized != 0 or pnl_pct != 0) else '0.00 (0.00%)'
            total_pnl_str = f'{total_pnl:.2f}' if total_pnl != 0 else '0.00'
            new_values = (symbol, status, mode, qty_str, cost_str, price_str, pnl_unrealized_str, total_pnl_str)
        ...
        if self.selected_symbol and self.selected_symbol in current_items and current_items[self.selected_symbol]:
            try:
                self.tree.selection_set(current_items[self.selected_symbol]);
                self.tree.see(current_items[self.selected_symbol])
            except:
                pass
        else:
            self.selected_symbol = None;
    ...
    def _on_select(self, event):
        self.selected_symbol = self.get_selected_symbol();
        self.on_select_callback(self.selected_symbol)
    ...
    def _on_right_click(self, event):
        ...
        if item:
            ...
            if values:
                symbol = values[0]
                bot_status = self.key_service.get_bot_status(self.selected_key_id, symbol)
                self.tree_menu.entryconfig('停止',
                                           state=tk.NORMAL if bot_status and bot_status['running'] else tk.DISABLED)
    ...
    def remove_selected(self):
        ...
        if messagebox.askyesno('确认', f'确定移除 {self.selected_symbol}？'):
            ...
            if self.log_callback:
                self.log_callback(f'已移除 {self.selected_symbol}')
    ...
    def stop_selected(self):
        ...
        if self.key_service.stop_bot(self.selected_key_id, self.selected_symbol):
            self.refresh()
            if self.log_callback:
                self.log_callback(f'{self.selected_symbol} 已停止')
        else:
            if self.log_callback:
                self.log_callback(f'{self.selected_symbol} 未运行')

    def force_sell_selected(self):
        if not self.selected_key_id or not self.selected_symbol:
            messagebox.showwarning('警告', '请先选中币种')
            return
        if messagebox.askyesno('确认', f'确定强制卖出 {self.selected_symbol} 的所有持仓吗？'):
            success = self.key_service.force_sell_symbol(self.selected_key_id, self.selected_symbol)
            if success:
                self.key_service.update_assets(self.selected_key_id, force=True)
                self.refresh()
                if self.log_callback:
                    self.log_callback(f'{self.selected_symbol} 强制卖出成功')
            else:
                if self.log_callback:
                    self.log_callback(f'{self.selected_symbol} 强制卖出失败，请检查日志')
